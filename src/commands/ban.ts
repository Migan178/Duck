module.exports = {
    ...
    run: async (client: any, msg: any, args: any) => {
        if (!msg.member.hasPermission("BAN_MEMBERS")) return msg.channel.send("당신은 이 명령어를 사용할 권한이 없습니다.");
        ...
        if (!mentionMember.bannable) return msg.channel.send("이사용자는 밴을 할수없습니다.");

        await mentionMember.ban({
            reason: reason
        }).then(() => msg.channel.send(`성공적으로 밴을 하였습니다. 밴한 사용자: ${mentionMember.user.tag}`));
    }
}